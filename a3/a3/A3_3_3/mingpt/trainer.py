...
class Trainer:
    ...
    def run(self):
        ...
        while True:
          ...
          #print(torch.sum((torch.argmax(logits,dim=1)== y))/len(y))

          self.train_acc=torch.sum((torch.argmax(logits,dim=1)== y))/len(y)
          ...
          self.val_acc=torch.sum((torch.argmax(logits,dim=1)== y))/len(y)
        ...
        model.eval()
        for i in range(ceil(len(self.validation_dataset)/config.batch_size)):
          try:
            batch = next(val_iter)
          except StopIteration:
            val_iter = iter(val_loader)
          ...
          logits, _ = model(x, y, self.downstream_finetune)
          self.final_acc+=torch.sum((torch.argmax(logits,dim=1)== y))

        self.final_acc= self.final_acc/len(self.validation_dataset)  
